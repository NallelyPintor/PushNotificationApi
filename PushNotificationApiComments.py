...
def stockExchange():
    ...
    for info in list:                           # Using for loop
        ...
        if regularMarketChangePercent > 0.5:
            ...
            print(regularMarketChangePercent)
schedule.every(10).seconds.do(stockExchange)    # show notifications every 6 hours, schedule.every(6).hours.do(stockExchange)
while(True):                                    # Make sure the function runs indefinitely
    schedule.run_pending()                      # Schedule pending execution
    time.sleep(1)
